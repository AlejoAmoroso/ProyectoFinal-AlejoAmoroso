...
function saveToLocalStorage() {
        let convertedAmount = sourceAmount.value;
        ...
        } else if (storedConvertedAmount <= 0) {
                formControlInputs.forEach(input => {
                        input.classList.add("error-border");
                })
                formControlHelpers.forEach(helper => {
                        helper.classList.add("form-control-helper-error")
                })
                Swal.fire({
                        icon: 'error',
                        title: 'La transferencia no pudo ser completada',
                        text: 'No posee fondos',
                      })
                return;
        }
        ...
        Swal.fire({
                ...
                text: 'El nuevo monto se vera reflejado en su balance'
              })
}